...
class MainActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        fragmentManager.beginTransaction().replace(R.id.displayed_screen_fl, TextViewFragment())
            .commit()
            ...
            navigationMenu.setNavigationItemSelectedListener {
                when (it.itemId) {
                    R.id.Lesson1 -> fragmentManager.beginTransaction().replace(
                        R.id.displayed_screen_fl, TextViewFragment()
                    ).commit()

                    R.id.Lesson2 -> fragmentManager.beginTransaction().replace(
                        R.id.displayed_screen_fl, Lesson2fragment()
                    ).commit()

                    R.id.Lesson3 -> fragmentManager.beginTransaction().replace(
                        R.id.displayed_screen_fl, Lesson3fragment()
                    ).commit()
                }
